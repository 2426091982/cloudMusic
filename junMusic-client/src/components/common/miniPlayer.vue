<template>
  <div>
    <div v-if="this.playObj.miniIsPlay && this.songDefault" class="mini_player">
      <div class="left" @click="hrefPlayMusic">
        <img
          ...
          v-lazy="this.picUrl + '?param=100y100'"
          alt=""
        />
      </div>
    ...
    <div v-if="this.playObj.largeIsPlay && this.songDefault" class="box">
      <!-- 音频 -->
      <audio
      ...
      <div class="record">
        <img
          class="rotateing"
          :class="isPlay ? 'running' : 'pause'"
          v-lazy="this.picUrl + '?param=200y200'"
          alt=""
        />
      </div>
      <!-- 控件 -->
...
export default {
  ...
  computed: {
    ...
    songDefault() {
      ...
      this.playObj.songList[this.playObj.activeIndex];
      // vueX的
  ...
  methods: {
    ...
    getAlbum() {
      this.$http("/song/detail", {
        ...
        // 封面跟请求的不相同才赋值
        if (this.picUrl != res.songs[0].al.picUr) {
